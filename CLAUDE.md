...
- Started-an-inactive-player callouts (added 2026-08-13, mechanism verified against real data, see below): a manager who started someone on bye, injured, or otherwise inactive, who scored 0 as a result. Must stay strictly retrospective per the tone clarification above.
  ...
  **Case 4, RESOLVED and built (2026-08-13): low-but-nonzero stats caused by a mid-game injury exit** (e.g. "3 points before leaving hurt, never came back" - looks like a bad game, isn't one). By default report the stat line neutrally, don't imply it was a bad performance either way, unless confirmed via `espn_api.injury_exits(event_id)`, which is now built and tested, not just designed:

  - **Mechanism:** scans that game's play-by-play (`drives` field on `summary?event=<id>`) for `"<TEAM>-<F.Lastname> was injured during the play"` and `"Injury Update: <TEAM>-<F.Lastname> has returned to the game"`, tracked in chronological play order so a player's *last* event (not just "were they ever tagged injured") determines the result. Team-prefixed keys avoid same-surname collisions between the two teams in a game.
  - **Verified against 4 real games** (2025 week 10: GB@PHI, MIA@BUF, TB@NE, CHI@NYG): correctly found real exits (GB-E.Jenkins, BUF-L.Jackson) with zero false positives, and correctly handled a player injured once but returned twice (TB-C.Barmore) by resolving to their last event rather than naively flagging on any injury mention.
  - **Four caveats resolved during this verification**, all confirmed either fixed by the chronological/team-keyed design or safe by construction, not just asserted:
    1. *Multiple injury events per player* - handled by last-event-wins tracking (validated on C.Barmore above).
    2. *Benched for non-injury reasons (blowout rest, coach's decision)* - structurally impossible to false-positive, the regex only matches literal injury-tagged play text, nothing else produces a match.
    3. *Team DEF/K slots* - DEF can never appear (defenses aren't individually injury-tagged in play text), kickers can appear if genuinely injured, which is correct, not a bug.
    4. *Same surname on both teams* - team-prefixed keys (e.g. "GB-E.Jenkins" vs a hypothetical "PHI-E.Jenkins") prevent collision, confirmed no issues across the 4 test games.
  - **Known data gap, confirmed across multiple real games, not a code bug:** ESPN sometimes logs a "has returned" update with no matching "was injured" tag ever appearing for that player. This makes the function under-count (a real exit could go undetected if ESPN never tags it) but never over-count, since a player only appears in the output with an explicit injury tag AND no later return tag. Fails toward "miss it," never "wrongly flag it," consistent with "if in doubt, leave it out."
  - **Weaker, supplementary signal, not built:** the game's full recap article text (`article.story` field on the same endpoint, ~8000 characters of real AP prose) sometimes explicitly mentions a notable injury exit. Not comprehensive, recap articles cover storylines, not every player's health. Not implemented since the play-by-play signal above is already comprehensive and verified; revisit only if the play-by-play approach misses something in practice.
